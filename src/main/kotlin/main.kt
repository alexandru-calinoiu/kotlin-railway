fun main() {
    val result = read() then ::parse then ::validAddress then ::notBlank then ::send
    result.onSuccess { println("All good: $it") }
...
fun parse(inputs: List<String>): Result<Email> =
    Result.success(Email(to = inputs.first(), subject = inputs[1], body = inputs[2]))
